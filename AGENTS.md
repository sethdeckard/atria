...
10. **Killed agents leave orphan panes.** When an agent process exits but the iTerm pane survives, the session ID stays live and the entry persists forever. Detect via: (a) consecutive unmatched screen reads transition status to idle (3 reads / ~9s to avoid flapping on normal output), then (b) session refresh checks if the iTerm session name no longer matches an agent pattern while idle, and removes it.
